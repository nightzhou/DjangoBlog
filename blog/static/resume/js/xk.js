...
function addClass(obj,className){
			if( obj.className == '' ){			//如果原来没有Class
				...
				var arrClassName = obj.className.split(' ');	//把obj的className字符串分割成数组
				var _index = arrIndexof( arrClassName,className );//详情见封装的arrIndexOf(arr.v)
		...
		function removeClass(obj,className){
			if(obj.className != ''	){							//如果原本有Class
				var arrClassName = obj.className.split(' ');	//那么就把原本的Class分割成数组形式
				var _index = arrIndexof( arrClassName,className );//详情见封装的arrIndexOf(arr.v)
				//如果有我们要移除的class
		...
		function arrIndexOf(arr,v){		//arr等于分割后的数组，v等于要本身有的Class
			for(var i=0;i<arr.length;i++){//开始循环数组
...
function drag(obj){
	obj.onmousedown = function(ev){	//1、先按下
		var ev = ev || event;			//兼容ev
		var disX = ev.clientX - this.offsetLeft; //获取鼠标到最左边的值减去盒子到最左边的值，等于属于距离盒子左边距离的值
		var disY = ev.clientY - this.offsetTop;//同上，只是是高度
